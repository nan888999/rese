      ...
      <div class="shop__contents">
        ...
        <div class="form-buttons">
          <div class="form-buttons__detail">
            <button onclick="location.href=''" class="common-btn">予約情報の確認</button>
            <button class="common-btn modal--open">編集</button>

            <!-- editモーダル　ここから -->
            <div class="easy-modal modal" id="edit-shop-modal">
              <div class="modal__content">
                <div class="modal__header">
                  <h1>Edit Shop</h1>
                  <span class="modal--close">×</span>
                ...
                <div class="modal__body">
                  <div class="edit-shop">
                    <form class="edit-shop-form" action="/admin/edit_shop" method="post">
                      ...
                      <input type="text" id="edit-shop-form__name" class="manage-shop-form__name" name="name" value="{{ old('name', $shop->name) }}">
                      <div class="form__error">
                        @error('name')
                          {{ $message }}
                        @enderror
                      ...
                      <select name="area_id" id="edit-shop-form__area" class="manage-shop-form__select">
                        <option value="" disabled class="option__title" selected>Area</option>
                        @foreach($areas as $area)
                          <option value="{{ $area->id }}"
                            @if(!empty($area_id) && $area_id == $area->id )
                      ...
                      <select name="category_id" id="edit-shop-form__category" class="manage-shop-form__select">
                        <option value="" disabled class="option__title" selected>Genre</option>
                        @foreach($categories as $category)
                          <option value="{{ $category->id }}"
                            @if(!empty($category_id) && $category_id == $category->id )
                      ...
                      <textarea id="edit-shop-form__detail" class="manage-shop-form__detail" name="detail">{{ old('detail', $shop->detail) }}</textarea>
                      <div class="form__error">
                        @error('detail')
                          {{ $message }}
                        @enderror
                      ...
                      <input type="url" id="edit-shop-form__img" class="manage-shop-form__img" name="img_url" value="{{ old('img-url', $shop->img_url) }}">
                      <div class="form__error">
                        @error('img_url')
                          {{ $message }}
                        @enderror
